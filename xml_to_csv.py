...
def main(TRAIN_DIRECTORY,TESTING_DIRECTORY,Trainset_path, Annotationset_path):
	
    for directory in [TRAIN_DIRECTORY,TESTING_DIRECTORY]:
        image_path = os.path.join(os.getcwd(), 'images/{}'.format(directory).format(directory))
        xml_df = xml_to_csv(image_path)
        xml_df.to_csv('data/{}_labels.csv'.format(directory), index=None)
        print('Successfully converted xml to csv.')
	 
	image_path = os.path.join(Trainset_path, 'images/train')
	xml_df = xml_to_csv(image_path)
